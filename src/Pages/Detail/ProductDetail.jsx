...
import rupiah from '../../Helpers/Utility';
...
const ProductDetail = () => {
    ...
    const addToCart = async (product) => {
        // if (setProceed) {
        try {
            let textConcat = `Halo saya ingin beli product ${product?.name} seharga ${rupiah(product?.price ?? 0)}, Berjumlah : ${productQuantity ?? 0}`
            window.location.href = `https://api.whatsapp.com/send?phone=6281398511932&text=${textConcat}`;
    ...
    return (
        <>
            <Container maxWidth='xl' >
                    ...
                    {loading ? (
                        ...
                        <section className='product-details'>
                            ...
                            <div style={{ display: 'flex', gap: 20 }}>
                                {/* <Typography variant="h6" color="red"><s> {rupiah((product.price > 1000 ? product.price + 1000 : product.price + 300) ?? 0)}</s> </Typography> */}
                ...
                {similarProduct && similarProduct.filter(prod => prod._id !== id) > 0 ? <Typography sx={{ marginTop: 10, marginBottom: 5, fontWeight: 'bold', textAlign: 'center' }}>Similar Products</Typography> : ""}
